...
const { ensureDir, copyDir, log, SKILL_ENGINEER_DIR, isSymlinkSupported, isWindows, CLAUDE_COMMAND_CONTENT } = require('./utils');
...
function installSkill(options) {
  ...
  for (const agent of resolved) {
    ...
    try {
      ...
      // For Claude Code, always ensure the slash-command entry exists in
      // .claude/commands/ so that /skill-engineer works — even if the skill
      // directory was already present from a prior version.
      if (agent.name === 'claude-code') {
        const commandsDir = path.join(path.dirname(agent.skillPath), 'commands');
        const commandFile = path.join(commandsDir, 'skill-engineer.md');
        if (!fs.existsSync(commandFile)) {
          ensureDir(commandsDir);
          fs.writeFileSync(commandFile, CLAUDE_COMMAND_CONTENT, 'utf8');
          log.success(`Created /skill-engineer command → ${commandFile}`);
        }
      }
    } catch (err) {
