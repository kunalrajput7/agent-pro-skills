...
function uninstallSkill(options) {
  ...
  for (const agent of resolved) {
    ...
    try {
      ...
      // For Claude Code, also remove the slash-command file.
      if (agent.name === 'claude-code') {
        const commandFile = path.join(path.dirname(agent.skillPath), 'commands', 'skill-engineer.md');
        if (fs.existsSync(commandFile)) {
          fs.unlinkSync(commandFile);
          log.success(`Removed /skill-engineer command from ${commandFile}`);
        }
      }
